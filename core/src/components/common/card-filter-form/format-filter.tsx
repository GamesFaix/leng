...
function sortFormats(formats: Format[]): Format[] {
  const withRanks = formats.map((f) => {
    return { format: f, rank: getSortRank(f) };
  });
...
const FormatFilter = (props: Props) => {
  const standardFormats: Format[] = useSelector(selectors.formats).map(
