...
const LangOption = (props: any, lang: Language, state: any) => {
  ...
  return (
    <li {...props} key={lang} classes={classes}>
      <FlagIcon lang={lang} />
      <div style={{ paddingLeft: "6px" }}>{lang}</div>
    </li>
  );
};
...
const FinishOption = (props: any, value: CardFinish, state: any) => {
  ...
  return (
    <li {...props} key={value} classes={classes}>
      <div style={{ paddingLeft: "6px" }}>{value}</div>
    </li>
  );
};
...
const CardForm = (props: Props) => {
  ...
  const cardVersionOptions = useSelector(
    ...
    .map((c) => {
      return { ...c, label: getVersionLabel(c) };
    })
    .sort(compareCards);
...
export const AddCardForm = (props: AddFormProps) =>
  CardForm({ ...props, card: null, title: "Add card" });
export const EditCardForm = (props: EditFormProps) =>
  CardForm({ ...props, title: "Edit card" });
